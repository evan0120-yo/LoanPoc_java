...
| # | 功能 | 說明 | 優先級 | 執行時機 |
|---|------|------|-------|--------|
| 1 | **接收申請** | 前端入口 | P0 | apply 時 |
| 2 | **黑名單過濾** | 在黑名單的人直接拒絕，**不存訂單** | P0 | ⭐ apply 時先查！|
| 3 | **Dedupe 去重** | 同一人短期內重複申請擋掉 | P0 | apply 時 |
| 4 | **建單** | 通過黑名單後才寫入 Outbox | P0 | apply 時 |
| 5 | **決策引擎** | 根據徵信資料決定 Pass/Reject | P0 | Bureau 回傳後 |
| 6 | **額度計算** | 根據收入、信用分數計算可貸額度 | P0 | 決策時 |
| 7 | **利率定價** | 根據風險等級決定利率 | P0 | 決策時 |
